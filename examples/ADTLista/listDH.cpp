...
struct list
{
    ...
    size_t *prev{nullptr}; // previous indexes array
 ...
 *         + sizeof(prev) i.e. sizeof(size_t *) usually 4B or 8B, independent of capacity!
...
// insert at front
bool insertFront(list &l, item i)
{
    ...
    else
    {
        ...
        l.prev[inserted] = NONE;    // no previous
        // update old front links (if any) and list
        if (l.front == NONE)
        {
        ...
        {
            l.front = l.prev[l.front] = inserted; // new previous and new front
        }
...
bool insertBack(list &l, item i)
{
    ...
    else
    {
        ...
        l.prev[inserted] = l.back; // link to  previous
...
bool removeFront(list &l, item &i)
{
    ...
    if (size(l) > 0)
    {
        ...
        else
        {
            ...
            l.prev[l.front] = NONE;    // new previous
        }
...
bool removeBack(list &l, item &i)
{
    ...
    if (size(l) > 0)
    {
        ...
        if (l.prev[removed] == NONE)
        {
            l.back = l.front = NONE; // new front and new back (empty list)
        }
        else
        {
            l.back = l.prev[removed]; // new back
            l.next[l.back] = NONE;    // new next
        }
        l.size--; // one element less
        // update free slots list
        l.next[removed] = l.free;
...
bool insertAt(list &l, item i, size_t p)
{
    ...
    else // general case: between two real nodes!!!
    {
        ...
        else
        {
            ...
            l.prev[inserted] = before;         // link to  previous
            // update contiguos links
            l.next[before] = inserted;           // new next
            l.prev[l.next[inserted]] = inserted; // new previous
            // update list
            l.size++; // one element more
            return true;
        }
...
bool removeFrom(list &l, item &i, size_t p)
{
    ...
    {
        // could be optimized (backward if p > size(l) / 2)
        // search index at removal point
        size_t removed = l.front;
        while (p-- > 0)
        {
            // move p times
            removed = l.next[removed];
        }
        // get data
        i = l.info[removed];
        // update contiguos links
        l.next[l.prev[removed]] = l.next[removed]; // new next
        l.prev[l.next[removed]] = l.prev[removed]; // new previous
        // update list
        l.size--; // one element less
        // update free slots list
        l.next[removed] = l.free;
...
bool insertOrdered(list &l, item i)
{
...
void clear(list &l)
{
    ...
    free(l.prev);
    l.prev = nullptr; // just to be on the safe side
    l.capacity = 0;
    // and empty the list
    l.size = 0;
...
bool extendCapacity(list &l, size_t newCapacity)
{
    ...
    if (pd != nullptr)
    {
        ...
        {
            // successfully extended!!!
            // resize the prev indexes array to newCapacity
            size_t *pp = (size_t *)realloc(l.prev, newCapacity * sizeof(l.prev[0]));
            if (pp != nullptr)
            {
                // successfully extended!!!
            ...
            else
            {
                ...
                // resize the next indexes array to old capacity
                l.next = (size_t *)realloc(l.next, l.capacity * sizeof(l.next[0]));
