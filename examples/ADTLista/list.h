...
bool insertFront(list &, item);          // insert at front
bool insertBack(list &, item);           // insert at back
bool removeFront(list &, item &);        // remove from front
bool removeBack(list &, item &);         // remove from back
bool insertAt(list &, item, size_t);     // insert at position (0 = front, size = back)
bool removeFrom(list &, item &, size_t); // remove from position (0 = front, size - 1 = back)
bool insertOrdered(list &, item);        // insert in order (requires < to be defined)
