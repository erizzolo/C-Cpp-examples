...
struct retta
{
    ...
    // rappresentazione: coefficiente angolare e intercetta
    //  y = m x + q (non valida per rette // all'asse y del tipo x = costante)
    coord m; // coefficiente angolare
    coord q; // intercetta
};
...
void print(const retta &r, ostream &out /* = cout */, bool newLine /* = true */)
{
    ...
    pretty(getC(r), out, ' ');
    out << " = 0; y = ";
    pretty(coefficienteAngolare(r), out, 'x');
    pretty(intercetta(r), out, ' ');
    out << ")";
    if (newLine)
    {
        out << endl;
