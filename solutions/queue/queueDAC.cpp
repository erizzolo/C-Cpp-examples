...
struct queue
{
    ...
    int capacity{0};     // actual capacity of the queue (max number of elements)
    int size{0};         // actual size of the queue (number of elements)
...
bool insert(queue &q, item i)
{
    ...
    else
    {
        ...
        int newCapacity = q.capacity == 0 ? 2 : q.capacity * 2;
        ...
        if (p != nullptr)
        {
            ...
            for (int pos = 0, newPos = q.capacity; pos < q.capacity; pos++, newPos++)
            {
                p[newPos] = p[pos];
            }
